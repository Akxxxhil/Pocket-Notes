...
function Dashboard() {
    ...
    function groupcreateHandler() {
        const newGroup = { id: `chat${notesgroup.length + 1}`, title: title, colorpicker: colorpicker };
        setNotesgroup((prevState) => [...prevState, newGroup]);
        setTilte(""); // Reset input value
        setPopup(false);
    }
                    ...
                    <div>
                        {notesgroup.map((eachGroup, index) => (
                            <div
                                ...
                                onClick={() => pageHandler(eachGroup.id)}
                            >
                                <span
                                    style={{
                                ...
                                ></span>
                                <span style={{ marginTop: "8px", marginLeft: "8px", cursor: "pointer" }}>
                                    {eachGroup.title}
                                </span>
                            </div>
                        ))}
